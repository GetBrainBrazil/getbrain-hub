...
type Form = {
  nome: string; tipo_pessoa: "PF" | "PJ"; cpf_cnpj: string; razao_social: string; nome_empresa: string;
...
const empty: Form = { nome: "", tipo_pessoa: "PJ", cpf_cnpj: "", razao_social: "", nome_empresa: "", emails: [], telefones: [], cep: "", estado: "", cidade: "", endereco: "", numero: "", bairro: "", complemento: "", observacoes: "", ativo: true };
...
export default function ClientesTab({ search }: { search: string }) {
  ...
  function startEdit() {
    ...
    setForm({
      ...
      cpf_cnpj: selected.cpf_cnpj || "", razao_social: selected.razao_social || "", nome_empresa: selected.nome_empresa || "",
  ...
  async function handleSave() {
    ...
    const payload: any = {
      ...
      nome_empresa: form.tipo_pessoa === "PJ" ? (form.nome_empresa || null) : null,
  ...
  function handleCopy() {
    ...
    if (selected.tipo_pessoa === "PJ" && selected.nome_empresa) text += `\nEmpresa: ${selected.nome_empresa}`;
    text += `\n${docLabel}: ${formatCpfCnpj(selected.cpf_cnpj, selected.tipo_pessoa)}\nE-mails: ${emails}\nTelefones: ${phones}`;
    const addr = buildAddressString(selected); if (addr) text += `\nEndereço: ${addr}`;
  ...
  if (mode === "view" && selected) {
    return (
      <FormPageShell
        ...
        subtitle="Visualize os dados do cliente"
        onBack={backToList}
        footer={
          <>
      ...
      >
        ...
        >
          ...
          {selected.tipo_pessoa === "PJ" && (
            <div className="grid grid-cols-2 gap-4">
              <DetailField label="Razão Social" value={selected.razao_social} />
              <DetailField label="Nome da Empresa" value={selected.nome_empresa} />
            </div>
          )}
        ...
        <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>Tem certeza que deseja excluir o cliente <strong>{selected.nome}</strong>? Esta ação não pode ser desfeita.</AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
  ...
  if (mode === "edit" || mode === "new") {
    return (
      <FormPageShell
        title={mode === "new" ? "Novo Cliente" : "Editar Cliente"}
        subtitle={mode === "new" ? "Preencha os dados do novo cliente" : "Atualize os dados do cliente"}
        onBack={cancelEdit}
        footer={
          <>
            <span />
      ...
      >
        <FormSection icon={FileText} title="Dados Principais">
          <div className="grid grid-cols-[2fr_3fr] gap-3">
            ...
            <div><Label>{form.tipo_pessoa === "PF" ? "CPF" : "CNPJ"}</Label><Input value={form.cpf_cnpj} onChange={e => setForm({ ...form, cpf_cnpj: applyCpfCnpjMask(e.target.value, form.tipo_pessoa) })} placeholder={form.tipo_pessoa === "PF" ? "000.000.000-00" : "00.000.000/0000-00"} /></div>
          </div>
          {form.tipo_pessoa === "PJ" ? (
            <>
              <div className="grid grid-cols-2 gap-3">
                <div><Label>Nome *</Label><Input value={form.nome} onChange={e => setForm({ ...form, nome: e.target.value })} placeholder="Cliente Teste" /></div>
              ...
              <div><Label>Nome da Empresa</Label><Input value={form.nome_empresa} onChange={e => setForm({ ...form, nome_empresa: e.target.value })} placeholder="Nome da empresa" /></div>
            </>
          ) : (
            <div><Label>Nome *</Label><Input value={form.nome} onChange={e => setForm({ ...form, nome: e.target.value })} placeholder="Cliente Teste" /></div>
          )}
        ...
        <FormSection icon={StickyNote} title="Observações">
          <div><Label>Observações</Label><Textarea rows={4} value={form.observacoes} onChange={e => setForm({ ...form, observacoes: e.target.value })} placeholder="Observações gerais sobre o cliente" /></div>
        </FormSection>
      </FormPageShell>
    );
  }
