...
import { StatusBadge, MaintenanceStatusBadge } from "@/components/projetos/ProjetoBadges";
...
export default function ProjetoDetalhe() {
  ...
  async function handleStatusChange(newStatus: ProjectStatus) {
    ...
    if (!confirm(`Mudar status para "${PROJECT_STATUS_OPTIONS.find((o) => o.value === newStatus)?.label}"?`)) return;
  ...
  async function handleSaveOverview() {
    ...
    if (criteria !== (project.acceptance_criteria ?? "")) updates.acceptance_criteria = criteria || null;
    if (notes !== (project.notes ?? "")) updates.notes = notes || null;
  ...
  if (!project) {
    return <div className="text-muted-foreground text-sm py-12 text-center">Carregando...</div>;
  }

  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="sm" onClick={() => navigate("/projetos")} className="gap-1">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            <span className="text-accent text-2xl font-mono font-semibold">{project.code}</span>
        ...
        <TabsContent value="overview" className="pt-4">
          <Card>
            <CardContent className="pt-6 space-y-3">
              ...
              <div className="pt-2">
                <Button onClick={handleSaveOverview}>Salvar alterações</Button>
              </div>
            </CardContent>
          </Card>
        </TabsContent>
