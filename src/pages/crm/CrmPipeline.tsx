...
function DraggableDeal({ deal, onOpen }: { deal: Deal; onOpen: () => void }) {
  ...
  return <div ref={setNodeRef} style={transform ? { transform: `translate3d(${transform.x}px, ${transform.y}px, 0)` } : undefined} {...attributes} {...listeners}><DealCard deal={deal} dragging={isDragging} onClick={() => !isDragging && onOpen()} /></div>;
}

function Column({ stage, deals, onOpen, onAdd }: { stage: DealStage; deals: Deal[]; onOpen: (deal: Deal) => void; onAdd: (stage: DealStage) => void }) {
  const { setNodeRef, isOver } = useDroppable({ id: stage });
  const total = deals.reduce((sum, d) => sum + Number(d.estimated_value ?? 0), 0);
  return <div className="flex w-80 shrink-0 flex-col rounded-lg bg-muted/20 p-3"><div className="mb-3 space-y-2 px-1"><div className="flex items-center justify-between gap-2"><div className="min-w-0"><h3 className="truncate text-sm font-semibold text-foreground">{DEAL_STAGE_LABEL[stage]}</h3><p className="text-xs text-muted-foreground">{formatCurrency(total)} · {deals.length} deals</p></div>{!stage.startsWith('fechado') && <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => onAdd(stage)}><Plus className="h-3.5 w-3.5" /></Button>}</div></div><div ref={setNodeRef} className={cn('flex max-h-[calc(100vh-360px)] min-h-[120px] flex-1 flex-col gap-2 overflow-y-auto rounded-md pr-1 transition-colors', isOver && 'bg-accent/10 ring-2 ring-accent/40')}>{deals.map((deal) => <DraggableDeal key={deal.id} deal={deal} onOpen={() => onOpen(deal)} />)}{deals.length === 0 && <button type="button" onClick={() => onAdd(stage)} className="rounded-md border border-dashed border-border/60 px-3 py-6 text-xs text-muted-foreground transition-colors hover:border-accent/50 hover:text-foreground">+ Novo deal</button>}</div></div>;
}
...
export default function CrmPipeline() {
  ...
  return <div className="space-y-5"><CrmKpiStrip metrics={metrics} />{deals.length === 0 ? <div className="rounded-lg border border-dashed border-border bg-muted/10 p-12 text-center"><p className="text-sm font-medium text-foreground">Nenhum deal no pipeline</p><p className="mt-1 text-xs text-muted-foreground">Crie um novo deal ou ajuste os filtros para visualizar.</p></div> : <DndContext sensors={sensors} onDragStart={(e: DragStartEvent) => setActiveId(String(e.active.id))} onDragEnd={handleDragEnd}><div className="flex gap-4 overflow-x-auto pb-4">{DEAL_STAGES.map((stage) => <Column key={stage} stage={stage} deals={grouped.get(stage) ?? []} onOpen={setSelected} onAdd={(s) => { setCreateStage(s); setCreateOpen(true); }} />)}</div><DragOverlay>{activeDeal && <DealCard deal={activeDeal} dragging />}</DragOverlay></DndContext>}<DealDrawer deal={selected} open={!!selected} onOpenChange={(v) => !v && setSelected(null)} /><NewDealDialog open={createOpen} onOpenChange={setCreateOpen} defaultStage={createStage} /><Dialog open={!!lost} onOpenChange={(v) => !v && setLost(null)}><DialogContent><DialogHeader><DialogTitle>Motivo da perda</DialogTitle></DialogHeader><div className="space-y-2"><Label>Informe o motivo para mover para perdido</Label><Textarea value={lostReason} onChange={(e) => setLostReason(e.target.value)} /></div><DialogFooter><Button variant="outline" onClick={() => setLost(null)}>Cancelar</Button><Button disabled={!lostReason.trim()} onClick={() => { if (lost) commitStage(lost.deal, lost.stage, { lost_reason: lostReason }); setLost(null); setLostReason(''); }}>Confirmar</Button></DialogFooter></DialogContent></Dialog><Dialog open={!!valueRequired} onOpenChange={(v) => !v && setValueRequired(null)}><DialogContent><DialogHeader><DialogTitle>Valor obrigatório</DialogTitle></DialogHeader><div className="space-y-2"><Label>Valor estimado do orçamento</Label><Input type="number" value={requiredValue} onChange={(e) => setRequiredValue(e.target.value)} /></div><DialogFooter><Button variant="outline" onClick={() => setValueRequired(null)}>Cancelar</Button><Button disabled={!requiredValue} onClick={() => { if (valueRequired) commitStage(valueRequired.deal, valueRequired.stage, { estimated_value: Number(requiredValue) }); setValueRequired(null); setRequiredValue(''); }}>Confirmar</Button></DialogFooter></DialogContent></Dialog><Dialog open={!!won} onOpenChange={(v) => !v && setWon(null)}><DialogContent><DialogHeader><DialogTitle>Deal ganho</DialogTitle></DialogHeader><p className="text-sm text-muted-foreground">Ótimo! O próximo passo será criar o projeto correspondente — isso estará disponível no 04B. Por enquanto, apenas marca como ganho.</p><DialogFooter><Button variant="outline" onClick={() => setWon(null)}>Cancelar</Button><Button onClick={() => { if (won) { commitStage(won.deal, won.stage); toast.success('Deal marcado como ganho'); } setWon(null); }}>Marcar como ganho</Button></DialogFooter></DialogContent></Dialog></div>;
}
