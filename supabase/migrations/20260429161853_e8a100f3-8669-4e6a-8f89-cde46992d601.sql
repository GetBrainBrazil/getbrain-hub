-- =========================================================
-- 1) Novos campos em deals: desconto, custos extras, MRR detalhado
-- =========================================================
ALTER TABLE public.deals
  ADD COLUMN IF NOT EXISTS discount_amount        numeric,
  ADD COLUMN IF NOT EXISTS discount_kind          text CHECK (discount_kind IN ('percent','fixed')),
  ...
  ADD COLUMN IF NOT EXISTS mrr_discount_value     numeric;

-- =========================================================
-- 2) Drop versões antigas da função (assinaturas variam)
-- =========================================================
DROP FUNCTION IF EXISTS public.close_deal_as_won(uuid, jsonb);
DROP FUNCTION IF EXISTS public.close_deal_as_won(uuid, jsonb, jsonb);

-- =========================================================
-- 3) Função reconstruída — v4 completa
-- =========================================================
...
BEGIN
  ...
  IF v_cliente_id IS NULL AND v_company_name IS NOT NULL THEN
    INSERT INTO public.clientes (
      ...
      v_company_name, v_company.legal_name, v_company.cnpj, 'PJ', true
    ) RETURNING id INTO v_cliente_id;
  END IF;

  -- Recorrência de implementação
  ...
  -- ============ MRR / Manutenção ============
  v_mrr_value := COALESCE(
    NULLIF(p_project_data->>'mrr_value','')::numeric,
    v_deal.estimated_mrr_value
  );
  v_mrr_start := COALESCE(
    NULLIF(p_project_data->>'mrr_start_date','')::date,
    v_deal.mrr_start_date,
    v_first_due
  );
  v_mrr_duration := COALESCE(
    NULLIF(p_project_data->>'mrr_duration_months','')::integer,
    v_deal.mrr_duration_months
  );
  v_mrr_discount_months := COALESCE(
    NULLIF(p_project_data->>'mrr_discount_months','')::integer,
    v_deal.mrr_discount_months,
    0
  );
  v_mrr_discount_value := COALESCE(
    NULLIF(p_project_data->>'mrr_discount_value','')::numeric,
    v_deal.mrr_discount_value,
    v_mrr_value
  );
  ...
  IF v_mrr_value IS NOT NULL AND v_mrr_value > 0 THEN
    INSERT INTO public.financial_recurrences (
      ...
      v_deal.organization_id,
      'Manutenção (MRR) — ' || v_project_name,
      'recurrence',
      'receita', v_mrr_value, 'mensal',
      v_mrr_start,
      v_mrr_duration,
      v_cliente_id, v_project_id,
      v_categoria_id, v_centro_custo_id, v_conta_bancaria_id, v_meio_pagamento_id,
    ...
    -- Aplica desconto nos primeiros N meses do MRR
    IF v_mrr_discount_months > 0 AND v_mrr_discount_value <> v_mrr_value THEN
      UPDATE public.movimentacoes m
       ...
       WHERE recurrence_id = v_mrr_recurrence_id
         AND installment_number <= v_mrr_discount_months;
    END IF;
    ...
    INSERT INTO public.maintenance_contracts (
      ...
      CASE WHEN v_mrr_discount_months > 0 AND v_mrr_value > 0
           ...
           ELSE 0 END,
      NULLIF(v_mrr_discount_months, 0),
      v_mrr_start,
      CASE WHEN v_mrr_duration IS NOT NULL
           THEN (v_mrr_start + (v_mrr_duration || ' months')::interval)::date
           ELSE NULL END,
  ...
  -- ============ Custos extras ============
  v_extra_costs := COALESCE(
    p_project_data->'extra_costs',
    v_deal.extra_costs,
    '[]'::jsonb
  );
  ...
  IF jsonb_typeof(v_extra_costs) = 'array' THEN
    ...
    LOOP
      DECLARE
        ...
        v_e_freq text;
      BEGIN
        IF v_e_amount IS NULL OR v_e_amount <= 0 OR v_e_desc IS NULL OR LENGTH(TRIM(v_e_desc)) = 0 THEN
          CONTINUE;
        END IF;
        ...
        ELSE
          -- Movimentação única de despesa
          INSERT INTO public.movimentacoes (
            descricao, tipo, valor_previsto,
  ...
  -- ============ Atualiza relação com a empresa ============
  ...
  RETURN jsonb_build_object(
    ...
    'implementation_total', v_implementation_total
  );
END;
$function$;

GRANT EXECUTE ON FUNCTION public.close_deal_as_won(uuid, jsonb, jsonb) TO authenticated;