-- =========================================================================
-- 1) Atualiza close_deal_as_won: cria MRR pausado quando trigger='on_delivery'
-- =========================================================================
...
BEGIN
  ...
  IF v_mrr_value IS NOT NULL AND v_mrr_value > 0 THEN
    ...
    -- Se cobrança começa "na entrega", recorrência fica pausada até trigger ativar
...
-- =========================================================================
-- 2) Trigger: ao mover projeto para "entregue", ativa MRR pausado e
--    preenche actual_delivery_date se necessário
-- =========================================================================
CREATE OR REPLACE FUNCTION public.activate_mrr_on_delivery()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  IF NEW.status = 'entregue' AND OLD.status IS DISTINCT FROM 'entregue' THEN
    -- Preenche actual_delivery_date se vazio
    IF NEW.actual_delivery_date IS NULL THEN
      NEW.actual_delivery_date := CURRENT_DATE;
    END IF;

    v_delivery_date := COALESCE(NEW.actual_delivery_date, CURRENT_DATE);

    -- Ativa recorrências MRR pausadas vinculadas a esse projeto cujo
    -- contexto comercial diz que a cobrança começa "na entrega"
    UPDATE public.financial_recurrences fr
       SET status = 'ativa',
           start_date = v_delivery_date,
           updated_at = now()
     WHERE fr.projeto_id = NEW.id
       AND fr.type = 'recorrente'
       AND fr.direction = 'entrada'
       AND fr.status = 'pausada'
       AND COALESCE(NEW.commercial_context->>'mrr_start_trigger','') = 'on_delivery';
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_activate_mrr_on_delivery ON public.projects;
CREATE TRIGGER trg_activate_mrr_on_delivery
BEFORE UPDATE OF status ON public.projects
FOR EACH ROW
EXECUTE FUNCTION public.activate_mrr_on_delivery();