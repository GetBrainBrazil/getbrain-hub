...
BEGIN
  ...
  -- Mantém uso do enum legacy project_type (deals.project_type), pois projects.project_type ainda usa o mesmo enum
  ...
  -- Cria projeto copiando todo escopo estruturado + baseline + source_deal_id
  ...
  -- Copia deal_dependencies → project_dependencies com mapeamento de enums
  FOR v_dep IN
    SELECT * FROM public.deal_dependencies
    WHERE deal_id = p_deal_id AND deleted_at IS NULL
  LOOP
    -- Mapeia dependency_type
    ...
    -- requested_from = "{nome} ({cargo})" ou só nome
    ...
    -- description com notas anexadas
