-- Adiciona campos de anexos comerciais ao projeto, herdados do deal
ALTER TABLE public.projects
  ADD COLUMN IF NOT EXISTS organograma_url text,
  ADD COLUMN IF NOT EXISTS mockup_url text,
  ADD COLUMN IF NOT EXISTS mockup_screenshots text[] NOT NULL DEFAULT '{}'::text[];

-- Estende close_deal_as_won para:
--  1. Copiar anexos comerciais (organograma, mockup) pro projeto criado
--  2. Vincular proposta aceita ao projeto criado (proposals.project_id)
