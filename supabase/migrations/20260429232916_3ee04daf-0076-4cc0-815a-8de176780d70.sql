...
CREATE OR REPLACE FUNCTION public.close_deal_as_won(p_deal_id uuid, p_project_data jsonb, p_installments jsonb DEFAULT '[]'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  -- Defaults globais (implementação) — fallback para MRR / extras quando não vierem específicos
  ...
  -- Categorização específica do MRR (com fallback p/ defaults globais)
  v_mrr_categoria_id      uuid := COALESCE(NULLIF((p_project_data->>'mrr_categoria_id')::text, '')::uuid, v_categoria_id);
  v_mrr_centro_custo_id   uuid := COALESCE(NULLIF((p_project_data->>'mrr_centro_custo_id')::text, '')::uuid, v_centro_custo_id);
  v_mrr_conta_bancaria_id uuid := COALESCE(NULLIF((p_project_data->>'mrr_conta_bancaria_id')::text, '')::uuid, v_conta_bancaria_id);
  v_mrr_meio_pagamento_id uuid := COALESCE(NULLIF((p_project_data->>'mrr_meio_pagamento_id')::text, '')::uuid, v_meio_pagamento_id);
  ...
  -- Counters
  v_installments_created      integer := 0;
  v_mrr_installments_created  integer := 0;
  v_extras_recurring          integer := 0;
  v_extras_once               integer := 0;
  v_tasks_created             integer := 0;
  v_proposal_marked_accepted  boolean := false;

  -- Per-row resolved categorization
  v_inst_cat   uuid; v_inst_cc uuid; v_inst_cb uuid; v_inst_mp uuid;
  v_ex_cat     uuid; v_ex_cc   uuid; v_ex_cb   uuid; v_ex_mp   uuid;
BEGIN
  ...
  -- Parcelas de implementação (per-parcela override + fallback global)
  FOR v_inst IN SELECT * FROM jsonb_array_elements(p_installments) LOOP
    v_inst_cat := COALESCE(NULLIF((v_inst->>'categoria_id')::text,'')::uuid, v_categoria_id);
    v_inst_cc  := COALESCE(NULLIF((v_inst->>'centro_custo_id')::text,'')::uuid, v_centro_custo_id);
    v_inst_cb  := COALESCE(NULLIF((v_inst->>'conta_bancaria_id')::text,'')::uuid, v_conta_bancaria_id);
    v_inst_mp  := COALESCE(NULLIF((v_inst->>'meio_pagamento_id')::text,'')::uuid, v_meio_pagamento_id);
    ...
    INSERT INTO public.financial_recurrences (
      ...
      v_inst_cat, v_inst_cc, v_inst_cb, v_inst_mp,
  ...
  IF v_mrr_value IS NOT NULL AND v_mrr_value > 0 THEN
    ...
    INSERT INTO public.financial_recurrences (
      ...
      v_mrr_months,
      v_cliente_id, v_project_id,
      v_mrr_categoria_id, v_mrr_centro_custo_id, v_mrr_conta_bancaria_id, v_mrr_meio_pagamento_id,
      'crm', 'deal', v_deal.id, 'ativa',
      v_disc_active,
    ...
    v_mrr_installments_created := 1;
  END IF;

  -- Custos extras (per-extra override + fallback global)
  FOR v_extra IN SELECT * FROM jsonb_array_elements(COALESCE(v_deal.extra_costs, '[]'::jsonb)) LOOP
    v_ex_cat := COALESCE(NULLIF((v_extra->>'categoria_id')::text,'')::uuid, v_categoria_id);
    v_ex_cc  := COALESCE(NULLIF((v_extra->>'centro_custo_id')::text,'')::uuid, v_centro_custo_id);
    v_ex_cb  := COALESCE(NULLIF((v_extra->>'conta_bancaria_id')::text,'')::uuid, v_conta_bancaria_id);
    v_ex_mp  := COALESCE(NULLIF((v_extra->>'meio_pagamento_id')::text,'')::uuid, v_meio_pagamento_id);
    ...
    INSERT INTO public.financial_recurrences (
      ...
      CURRENT_DATE,
      v_cliente_id, v_project_id,
      v_ex_cat, v_ex_cc, v_ex_cb, v_ex_mp,
      'crm', 'deal', v_deal.id, 'ativa'
    );

    IF v_extra->>'recurrence' = 'once' THEN
  ...
  RETURN jsonb_build_object(
    ...
    'mrr_installments_created', v_mrr_installments_created,
    'extras_recurring', v_extras_recurring,
    'extras_once', v_extras_once,
    'tasks_created', v_tasks_created,
    'proposal_marked_accepted', v_proposal_marked_accepted
  );
END;
$function$;
