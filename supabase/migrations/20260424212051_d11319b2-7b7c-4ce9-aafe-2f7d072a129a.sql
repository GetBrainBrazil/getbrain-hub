...
SELECT p.id AS project_id,
    ...
    -- IMPLEMENTAÇÃO: classificada pela categoria 'Implementação'
    COALESCE(( SELECT sum(COALESCE(NULLIF(m.valor_realizado, 0::numeric), m.valor_previsto))
           FROM movimentacoes m
          WHERE (m.projeto_id = p.id
                 OR (m.source_entity_id = p.id AND m.source_entity_type = ANY (ARRAY['project'::text, 'maintenance_contract'::text])))
            AND m.categoria_id = '54986866-7bd2-42e5-abec-2378b3e7c6a9'::uuid
    ...
    -- MANUTENÇÃO: categoria 'Manutenção' OU lançamentos auto-gerados pelo contrato (fallback p/ categoria nula)
    COALESCE(( SELECT sum(COALESCE(NULLIF(m.valor_realizado, 0::numeric), m.valor_previsto))
           FROM movimentacoes m
          WHERE (m.projeto_id = p.id
                 OR (m.source_entity_id = p.id AND m.source_entity_type = ANY (ARRAY['project'::text, 'maintenance_contract'::text])))
            AND (m.categoria_id = 'fd3b23be-1101-4122-83d1-6b559c64c04b'::uuid
                 OR (m.categoria_id IS NULL AND m.source_entity_type = 'maintenance_contract'::text))
